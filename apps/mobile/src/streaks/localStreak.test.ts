...
describe("computeLocalStreak", () => {
  ...
  it("breaks current streak after a missed day", () => {
    const day = (ymd: string) => Date.parse(`${ymd}T12:00:00Z`);
    const events = [wake(day("2024-01-01")), wake(day("2024-01-03"))];
    const s = computeLocalStreak(events, day("2024-01-05"), "UTC");
    assert.equal(s.current, 0);
    assert.equal(s.longest, 1);
    assert.equal(s.totalWins, 2);
  });

  it("dedupes multiple wakes on the same calendar day", () => {
    const day = (ymd: string) => Date.parse(`${ymd}T12:00:00Z`);
    const events = [
      wake(day("2024-01-02") + 1000, "a"),
      wake(day("2024-01-02") + 2000, "b"),
      wake(day("2024-01-01"), "c"),
    ];
    const s = computeLocalStreak(events, day("2024-01-02"), "UTC");
    assert.equal(s.current, 2);
    assert.equal(s.totalWins, 2);
  });
});
