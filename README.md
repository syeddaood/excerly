...
1. Android alarm reliability. Alarm fires at the exact time, over the lock screen, on a Samsung device with battery optimization at defaults, after a reboot, in Doze. Do not proceed until this is bulletproof on real hardware.
2. Permission onboarding flow. Measure drop-off. This is where you lose half your users.
3. Missions (QR scan first — simplest and most effective).
4. Focus window with ritual screens. Soft enforcement only.
5. Android hard enforcement (overlay).
6. iOS, mirroring 1–4.
7. iOS shield, if and only if the entitlement is granted.
